...
import com.study.spring_data_jpa.dto.MemberDto;
...
public interface MemberSpringDataJpaRepository extends JpaRepository<Member, Long>, MemberSpringDataJpaCustomRepository {
     ...
     * Member entity에 선언
     *
     * @NamedQuery(name="Member.searchUser", query="select m from Member m where username= :username"
     * )
     **/
     ...
     ************************************************************************************************************/
    /**
     * @Query 스프링 데이터 JPA를 사용하면 실무에서 Named Query를 직접 등록해서 사용하는 일은 드물다.
     ...
     * DTO로 직접 조회
     **/
    @Query("select new com.study.spring_data_jpa.dto.MemberDto(m.id, m.username, t.name) from Member m join m.team t")
    ...
    /**
     * count 쿼리 분리
     * 분리이유는 count 쿼리로 인해 성능 저하를 일으킬 수 있다.
     * 따라서 driving table에만 count(*)를 걸어서 성능 저하를 막을 수 있다.
     **/
     ...
     ************************************************************************************************************/
    @Modifying(clearAutomatically = true) //벌크 수정, 영속성 컨텍스트 초기화 / @Modifying 어노테이션은 executeUpdate() 함수를 호출하는것과 같다.
