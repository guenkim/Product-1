package com.study.queryDsl.repository.queryDsl.repository;
...
import com.study.queryDSL.entitiy.QSubselect;
import com.study.spring_data_jpa.entitiy.QTeam;
...
public class QuerydslBasicTest {
     ...
     *          QMember qMember = QMember.member; //기본 인스턴스 사용 > 추천
     *************************************************************************************************************/
    @Test
    ...
    public void fetchJoinUse() throws Exception {
        ...
        Member findMember = queryFactory
                ...
                .fetchOne();
        boolean loaded =
                emf.getPersistenceUnitUtil().isLoaded(findMember.getTeam().getName());
    ...
    /*************************************************************************************************************
     ...
     3.from(join) 절에 subquery : @Subselect
     *************************************************************************************************************/
    ...
    public void subquery() {
        ...
        List<Tuple> fetch = queryFactory
                .select(member.username,
                        QSubselect.subselect.id,
                        QSubselect.subselect.username,
                        QSubselect.subselect.team_id,
                        QSubselect.subselect.name,
                        /*****************************
                         select 절에 subquery
                         ******************************/
                        JPAExpressions
                                .select(QSubselect.subselect.age.avg())
                                .from(QSubselect.subselect)
                                .join(member)
                                .on(member.id.eq(QSubselect.subselect.id))
                ).from(member)
                /*****************************
                 from 절에 subquery @Subselect
                 ******************************/
                .join(QSubselect.subselect)
                .on(member.id.eq(QSubselect.subselect.id))
                /*****************************
                 where 절에 subquery
                 ******************************/
